...
def motion_axis_from_denoised(denoised, eps=1e-8):
    """Estimate a global motion axis from frame-to-frame denoised residuals.
...
def _energy_map(v: torch.Tensor) -> torch.Tensor:
...
def sample(
    ...
    trace = {
        ...
        "records": [],
        "sampler_note": "diagnostics are taken at the released backward-fusion point, not from a separate same-latent probe",
    }
    probed_count = 0
    ...
    with torch.no_grad():
        with torch.autocast(device):
            ...
            def CFG(x_u, x_c, scale):
                ...
                return rearrange(x_u + scale_tensor * (x_c - x_u), "b t ... -> (b t) ...")
            ...
            for i in tqdm(model.sampler.get_sigma_gen(num_sigmas), total=num_sigmas - 1):
                ...
                if i < 0.2 * num_steps:
                    ...
                    if should_probe:
                        ...
                        norm_bwd = _norm(d_bwd)
                        norm_aligned = _norm(d_bwd_aligned)
                        norm_orth = _norm(d_orth)
                        norm_fused = _norm(d_fused)
                        ...
                        record = {
                            ...
                            "cos_bwd_axis": float(_cos(d_bwd, m)),
                            "cos_fused_axis": float(_cos(d_fused, m)),
                            "cos_bwd_aligned": float(_cos(d_bwd, d_bwd_aligned)),
                            "cos_fused_aligned": float(_cos(d_fused, d_bwd_aligned)),
                            "aligned_ratio": float(norm_aligned / (norm_bwd + 1e-8)),
                            "orth_ratio": float(norm_orth / (norm_bwd + 1e-8)),
                            "fused_ratio": float(norm_fused / (norm_bwd + 1e-8)),
                            "start_residual_to_axis_cos": start_stats["cos"],
                            "end_residual_to_axis_cos": end_stats["cos"],
                            "start_residual_norms": start_stats["norm"],
                            "end_residual_norms": end_stats["norm"],
                        }
                        if store_energy_maps:
                            record["raw_energy"] = _energy_map(d_bwd).detach().cpu().to(torch.float16)
                            record["orth_energy"] = _energy_map(d_orth).detach().cpu().to(torch.float16)
                            record["aligned_energy"] = _energy_map(d_bwd_aligned).detach().cpu().to(torch.float16)
                            record["fused_energy"] = _energy_map(d_fused).detach().cpu().to(torch.float16)
    ...
    if save_alignment_debug:
        ...
        print(f"[alignment-probe] saved trace to: {trace_path}")
        print(f"[alignment-probe] generated gif: {video_path}")
